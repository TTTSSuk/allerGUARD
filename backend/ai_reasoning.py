...
def create_analysis_prompt(allergen_details, all_ingredients):
    """สร้าง Prompt สำหรับให้ AI วิเคราะห์"""
    ...
    # รายการสารที่ตรวจพบ
    # detected_list = "\n".join([
    #     f"- {item['ingredient']} (ตำแหน่ง {item['position']}/{item['total']}, ความเข้มข้น: {item['concentration']})"
...
#    [อธิบายว่าทำไมสารนี้จึงอันตรายสำหรับผู้ที่แพ้ โดยเฉพาะจากตำแหน่งและความเข้มข้น]
    prompt = f"""คุณเป็นผู้เชี่ยวชาญด้านผิวหนังและเครื่องสำอาง

📋 ส่วนผสมทั้งหมดในผลิตภัณฑ์ (เรียงตามความเข้มข้น จากมากไปน้อย):
{ingredients_list}

⚠️ ผู้ใช้แพ้สารเหล่านี้ที่พบในผลิตภัณฑ์:
{detected_list}
